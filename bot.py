    ...
    try:
        resp = ai.messages.create(
            ...
            system=(
                ...
                "השעיה, ביטול משחק או חדשות דחופות. אחרת ענה NO."
            ),
...
def summarize(chat_name: str, messages: list[str]) -> str:
    try:
        ...
        resp = ai.messages.create(
            ...
            messages=[{"role": "user", "content": f"סכם את ההודעות מהקבוצה '{chat_name}':\n{joined}"}],
        )
        return resp.content[0].text.strip()
...
async def main():
    ...
    async def on_message(event):
        ...
        if is_important(text):
            ...
            alert = f"🔔 *התראה חשובה* מ־{chat_name}\n🏷️ [{tag}]\n\n{text[:600]}"
        ...
        if len(pending_messages[chat_id]) >= SUMMARY_EVERY:
            ...
            report  = f"📋 *סיכום* מ־{chat_name} ({now})\n\n{summary}"
